...
const logoTaglineEl = document.querySelector('.logo .logo-tagline');
const logoNameEl = document.querySelector('.logo .logo-name');
const relatedEl = document.querySelector('.related');
const relatedItems = relatedEl.querySelectorAll('.grid__item');

// Cache original logo texts once
const logoTaglineText = logoTaglineEl.textContent;
const logoNameText = logoNameEl.textContent;

// Store original text content
textElements.forEach((el) => {
  el.dataset.text = el.textContent;
});
logoTaglineEl.dataset.text = logoTaglineText;
logoNameEl.dataset.text = logoNameText;
...
function initScramble() {
  ...
  // Scramble logo tagline and name once on init (not scroll-based)
  scramble(logoTaglineEl, { revealDelay: 0.5 });
  scramble(logoNameEl, { revealDelay: 0.5 });
}
...
function animateRelatedEnter() {
  gsap.to([logoTaglineEl, logoNameEl], relatedConfig.logo.fadeOut);
  gsap.fromTo(
    relatedItems,
...
function animateRelatedLeave() {
  gsap.to([logoTaglineEl, logoNameEl], relatedConfig.logo.fadeIn);
